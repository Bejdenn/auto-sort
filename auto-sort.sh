...
cd "${SOURCE}" || echo "Error: source folder not found" >&2

echo "Start process"

# Count files
FILES_COUNTER=$(find -- *.* 2> /dev/null | wc -l | xargs)
    ...
    for FILE in "${FILES_ARR[@]}"; do
        ...
        if [[ ${ALLOWED_EXT} == *"$EXT"* ]]; then
            ...
            if [[ -z ${DATETIME} ]]; then
                ...
                SOURCE_FILE_BASE64="$(base64 "${FILE}")"
                TARGET_FILE_BASE64="$(base64 "${TARGET_FILEPATH}")"
